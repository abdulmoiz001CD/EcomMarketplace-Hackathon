...
const OfferCard = ({ imageSrc, title, price }: { imageSrc: string; title: string; price: string }) => (
  <div className="flex justify-between w-full sm:w-[267px] h-[74px] relative">
    <div className='relative w-[107px] h-[74px] bg-[#F7F7F7]'>
      <Image 
        src={imageSrc} 
        alt={title} 
        fill
        className="object-contain p-2"
        sizes="(max-width: 768px) 107px, 107px"
      />
    </div>
    <div className="flex flex-col justify-center flex-1 pl-4">
      <h3 className="text-[14px] md:text-[10px] xl:text-[16px] font-semibold">{title}</h3>
      <p className="text-[11px] md:text-[9px] xl:text-[12px] text-gray-600">{price}</p>
    </div>
  </div>
);
...
const Discount = () => {
  return (
    <section className="flex flex-col xl:flex-row items-center gap-9 xl:gap-3 justify-between max-w-[1177px] mx-auto py-8 sm:py-12">
      ...
      <div className='xl:max-w-[267px] px-[3rem] xl:px-0 w-full flex flex-col sm:flex-row xl:flex-col gap-4 sm:gap-6 xl:gap-8 justify-between items-center min-h-[270px] xl:pb-9'>
        <OfferCard 
          imageSrc="./images/offCard1.svg"
          title="Executive Seat chair"
          price="$32.00"
        />
        <OfferCard 
          imageSrc="./images/offCard2.svg"
          title="Executive Seat chair"
          price="$32.00"
        />
        <OfferCard 
          imageSrc="./images/offCard3.svg"
          title="Executive Seat chair"
          price="$32.00"
        />
      </div>
